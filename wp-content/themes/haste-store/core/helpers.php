	...
	if ( ! isset( $_GET[ 'infinity' ] ) ) {
		...
		if ( $total_pages > 1 ) {
			...
			$arguments = apply_filters( 'odin_pagination_args', array(
					...
					'mid_size'  => $mid,
					'type'      => 'list',
					'prev_text' => __( '&laquo; Previous', 'haste-store' ),
					'next_text' => __( 'Next &raquo;', 'haste-store' ),
				)
			);
...
function odin_breadcrumbs( $homepage = '' ) {
	...
	! empty( $homepage ) || $homepage = __( 'Home', 'haste-store' );
	...
	if ( ! is_home() && ! is_front_page() || is_paged() ) {
		...
		// Category archive.
			...
			printf( __( '%sCategory: %s%s', 'haste-store' ), $current_before, single_cat_title( '', false ), $current_after );

		// Tags archive.
		} elseif ( is_tag() ) {
			printf( __( '%sTag: %s%s', 'haste-store' ), $current_before, single_tag_title( '', false ), $current_after );
		...
		// Search page.
			...
			printf( __( '%sSearch result for: &quot;%s&quot;%s', 'haste-store' ), $current_before, get_search_query(), $current_after );

		// Author archive.
		} elseif ( is_author() ) {
			$userdata = get_userdata( $author );

			echo $current_before . __( 'Posted by', 'haste-store' ) . ' ' . $userdata->display_name . $current_after;
		...
		// 404 page.
			...
			echo $current_before . __( '404 Error', 'haste-store' ) . $current_after;
		}
