...
class Haste_Content_Image_Module extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__( 'Haste Content and Image Module', 'haste-store' ),
			array( 'description' => __( 'Displays title, content (shotcodes included), a button and image as background, with few options.', 'haste-store' ), 'customize_selective_refresh' => true, )
		);
	}
	...
	public function form( $instance ) {
		$title         = isset( $instance['title'] ) ? $instance['title'] : __('My module title', 'haste-store');
		...
		?>
		<h3><?php _e( 'Content options', 'haste-store' ); ?></h3>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>">
				<?php _e( 'Title:', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'btn_text' ); ?>">
				<?php _e( 'Button text', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_name( 'btn_link' ); ?>"><?php _e( 'Button link:', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'btn_class' ); ?>">
				<?php _e( 'Button CSS classes', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'align' ); ?>">
				...
				<?php _e( 'Content alignment', 'haste-store' ); ?>
				...
				<select class='widefat' id="<?php echo $this->get_field_id('align'); ?>"
					...
					<option
						...
						<?php _e( 'Content centered', 'haste-store' ); ?>
					</option>
					<option
						value='content-left'
						<?php echo ( $align == 'content-left' ) ? 'selected' : ''; ?>>
						<?php _e( 'Content on left', 'haste-store' ); ?>
					</option>
					<option
						value='content-right'
						<?php echo ( $align == 'content-right' ) ? 'selected' : ''; ?>>
						<?php _e( 'Content on right', 'haste-store' ); ?>
					</option>
				</select>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'padding' ); ?>">
				<?php _e( 'Area padding (%)', 'haste-store' ); ?>
		...
		<hr />
		<h3><?php _e( 'Background options', 'haste-store' ); ?></h3>
		<p class="upload-img">
			<div class='image-preview-wrapper'>
				<img class='image-preview' src='<?php echo esc_attr( $image ); ?>' width='100%' height='100' style='max-height: 100px; width: 100px;'>
			</div>
			<label for="<?php echo $this->get_field_name( 'image' ); ?>"><?php _e( 'Image:', 'haste-store' ); ?></label>
			...
			<input class="upload_image_button" type="button" value="<?php _e( 'Set image ', 'haste-store' ); ?>" />
			<input class="clear_image_button" type="button" value="<?php _e( 'Remove', 'haste-store' ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'img_pos_v' ); ?>">
				<?php _e( 'Image vertical position (%)', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'img_pos_h' ); ?>">
				<?php _e( 'Image horizontal position (%)', 'haste-store' ); ?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'cover' ); ?>">
				<input id="<?php echo $this->get_field_id( 'cover' ); ?>" name="<?php echo $this->get_field_name( 'cover' ); ?>" type="checkbox" value="1" <?php checked( 1, $cover, true ); ?> /> <?php _e( 'Expand image to cover the entire module area', 'haste-store' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'bg_fix' ); ?>">
				<input id="<?php echo $this->get_field_id( 'bg_fix' ); ?>" name="<?php echo $this->get_field_name( 'bg_fix' ); ?>" type="checkbox" value="1" <?php checked( 1, $cover, true ); ?> /> <?php _e( 'Fix the background image', 'haste-store' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'bg_repeat' ); ?>">

				<?php _e( 'Background image repeat', 'haste-store' ); ?>
				...
				<select class='widefat' id="<?php echo $this->get_field_id('bg_repeat'); ?>"
					...
					<option
						...
						<?php _e( 'No repeat', 'haste-store' ); ?>
					</option>
					<option
						value='repeat'
						<?php echo ( $align =='repeat' ) ? 'selected' : ''; ?>>
						<?php _e( 'Vertical and horizontal repeat', 'haste-store' ); ?>
					</option>
					<option
						value='repeat-x'
						<?php echo ( $align =='repeat-x' ) ? 'selected' : ''; ?>>
						<?php _e( 'Horizontal repeat', 'haste-store' ); ?>
					</option>
					<option
						value='repeat-y'
						<?php echo ( $align =='repeat-y' ) ? 'selected' : ''; ?>>
						<?php _e( 'Vertical repeat', 'haste-store' ); ?>
					</option>
				</select>
			</label>
		</p>
		<p>
            <label for="<?php echo $this->get_field_id( 'bg_color' ); ?>">
				<?php _e( 'Background Color', 'haste-store' ); ?>
			</label>
