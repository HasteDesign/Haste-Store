...
class Haste_Posts_Module extends WP_Widget {
	...
	*/
	public function __construct() {
		parent::__construct(
			'haste_posts_module',
			__( 'Haste Posts Module', 'haste-store' ),
			array( 'description' => __( 'Display blog posts.', 'haste-store' ), 'customize_selective_refresh' => true, )
		);
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'posts_per_page' ); ?>">
				<?php _e( 'Number of posts to display (use -1 to show all posts)', 'haste-store' ); ?>
		...
		<h3><?php _e( 'Post selection criteria', 'haste-store' ); ?></h3>
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>">
				<?php _e( 'From a category (category slug)', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'keyword' ); ?>">
				<?php _e( 'From keyword', 'haste-store' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order_by' ); ?>">
				...
				<?php _e( 'Order posts by', 'haste-store' ); ?>
			...
			<label for="<?php echo $this->get_field_id( 'order' ); ?>">
				...
				<select class='widefat' id="<?php echo $this->get_field_id('order'); ?>"
					...
					<option
						...
						<?php _e( 'Descending', 'haste-store' ); ?>
					</option>
					<option
						value='ASC'
						<?php echo ( $order == 'ASC' ) ? 'selected' : ''; ?>>
						<?php _e( 'Ascending', 'haste-store' ); ?>
					</option>
				</select>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'posts_classes' ); ?>">
				<?php _e( 'CSS posts classes', 'haste-store' ); ?>
		...
		<h3><?php _e( 'Post display options', 'haste-store' ); ?></h3>
		<p>
			<label for="<?php echo $this->get_field_id( 'display_thumb' ); ?>">
				<input id="<?php echo $this->get_field_id( 'display_thumb' ); ?>" name="<?php echo $this->get_field_name( 'display_thumb' ); ?>" type="checkbox" value="1" <?php checked( 1, $display_thumb, true ); ?> /> <?php _e( 'Display post thumbnails', 'haste-store' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'display_excerpt' ); ?>">
				<input id="<?php echo $this->get_field_id( 'display_excerpt' ); ?>" name="<?php echo $this->get_field_name( 'display_excerpt' ); ?>" type="checkbox" value="1" <?php checked( 1, $display_excerpt, true ); ?> /> <?php _e( 'Display post excerpts', 'haste-store' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'display_date' ); ?>">
				<input id="<?php echo $this->get_field_id( 'display_date' ); ?>" name="<?php echo $this->get_field_name( 'display_date' ); ?>" type="checkbox" value="1" <?php checked( 1, $display_date, true ); ?> /> <?php _e( 'Display post dates', 'haste-store' ); ?>
			</label>
		</p>
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="widget-posts">';
			...
			wp_reset_postdata();
				 ...
				 _e( 'Sorry, no posts matched your criteria.', 'haste-store' );
