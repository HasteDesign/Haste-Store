 ...
 * @return string
 */
function haste_store_get_image_url( $id, $width, $height, $crop = true, $upscale = false ) {
 ...
 */
function haste_store_thumbnail( $width, $height, $alt, $crop = true, $class = '', $upscale = false ) {
	...
	if ( $thumb ) {
		$image = haste_store_get_image_url( $thumb, $width, $height, $crop, $upscale );
		...
		return apply_filters( 'haste_store_thumbnail_html', $html );
	}
}
 ...
 *
 * Usage:
 * <?php echo haste_store_get_term_meta( $term_id, $field );?>
 *
 * @since  2.2.7
 *
 ...
 */
function haste_store_get_term_meta( $term_id, $field ) {
	...
	$option_name = sprintf( 'haste_store_term_meta_%s_%s', $term_id, $field );
 ...
 *
 * Usage:
 * Place: <?php echo haste_store_excerpt( 'excerpt', value ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function haste_store_excerpt( $type = 'excerpt', $limit = 40 ) {
	$limit = (int) $limit;
 ...
 * Add in single.php <?php haste_store_related_posts(); ?>
 * To show related by tags:
 * Add in single.php <?php haste_store_related_posts( 'tag' ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function haste_store_related_posts( $display = 'category', $qty = 4, $title = '', $thumb = true, $post_type = 'post' ) {
	global $post;

	$show = false;
	$post_qty = (int) $qty;
	! empty( $title ) || $title = __( 'Related Posts', 'haste-store' );
		...
		if ( $related->have_posts() ) {
			...
			while ( $related->have_posts() ) {
				...
				if ( $thumb ) {
					...
					$image = apply_filters( 'haste_store_related_posts_thumbnail', $img );
